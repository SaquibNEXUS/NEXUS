...
function App() {
  ...
  // The top navbar (NEXUS logo, Home, Explore, My Activity, Profile) is only
  // usable once the student has actually entered the Mission Workspace.
  // Before that — on the initial Home screen ("briefing") or the Mission 001
  // briefing screen ("home") — the intended path is Enter Beta → Continue,
  // not the navbar. Returning to either of those screens from inside the
  // mission re-locks the navbar until Continue is used again. None of this
  // touches persisted findings/progress — it only guards which button
  // presses are allowed to change `view`.
